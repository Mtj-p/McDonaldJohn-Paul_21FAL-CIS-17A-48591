...
Stats *stat(const Array *array){
    ...
    float medi1;
    if(medi%2==0){
        medi1=(array->data[medi/2-1]+array->data[(medi/2)]);
        medi1=medi1/2;
    }
    if(medi%2!=0){
        medi1=(array->data[(medi/2)]);
    }
    stats->median=medi1;
    ...
    for(int i=0;i<arySize-1;i++){
        ...
        if(array->data[i]==array->data[i+1]){
            ...
            count++;
            i++;
            while(array->data[i]==array->data[i+1]){
                count++;
                i++;
            }
    ...
    stats->mode->size=maxFreq;
    int nModes=modes;
    if(nModes!=0)stats->mode->data=new int[nModes];
    for(int i=0;i<mAryC;i++){
        stats->mode->data[i]=mAry[i];
    }
    stats->modFreq=modes;
